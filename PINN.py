...
class BurgerSupervisor:
    ...
    def train(self, train, val):
        ...
        for i in range(self.epochs):
            ...
            if val_f_loss < best_ls:
                self.net.save_weights('./saved_model/PINN_weights_best')
...
def plotter(x_, u_, name, NUMX, NUMT):
    ...
    def animate(t):
        line.set_ydata(u_[:, t])
        text.set_text('t= %f' % T[t])
        return line, text

    ani = animation.FuncAnimation(fig, animate, frames=u_.shape[1], interval=50)
    plt.tight_layout()
    ani.save('./Burgers_' + name + '.gif', writer='imagemagick', fps=20)
...
if __name__ == "__main__":
    ...
    net = PINN(num_layers=18, hidden_dim=18, output_dim=1, act_fn='sigmoid')
    sup = BurgerSupervisor(nu=NU, net=net, epochs=281, lr=0.016580695, alpha=0.016580695) 
    train, val, test = get_data(NT=STEPS, NX=N, X_U=1., X_L=-1., T_max=1., Nu=NU)
    sup.train(train,val)
    plotter(test['x'], net(test['x'][:,0], test['x'][:,1]).numpy(), "test_tune", N, STEPS)
